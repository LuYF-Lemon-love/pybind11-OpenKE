...
void read_train_files() {
    ...
    tph = (REAL *)calloc(relation_total, sizeof(REAL));
    ...
    for (INT i = 0; i < relation_total; i++) {
        tph[i] = freq_rel.at(i) / heads_rel.at(i);
...
INT *begin_head_type, *end_head_type, *begin_tail_type, *end_tail_type;
...
void read_type_files() {
    ...
    end_tail_type = (INT *)calloc(relation_total, sizeof(INT));
    ...
    for (INT i = 0; i < relation_total; i++) {
        ...
        std::sort(head_type_rel + begin_head_type[rel], head_type_rel + end_head_type[rel]);
        istrm >> rel >> tot;
